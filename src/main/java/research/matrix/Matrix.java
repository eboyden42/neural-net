...
public class Matrix implements Serializable {
	...
	public Matrix(int rows, int cols, double[] values) {
		...
		Matrix tmp = new Matrix(cols, rows, i->values[i]);
	...
	public Matrix multiply(Matrix m) {
		...
		assert cols == m.rows : "Multiplication Impossible: Matrix 1 cols does not equal Matrix 2 rows";
	...
	public Matrix getGreatestRowNumber() {
		...
		double[] greatestColNumber = new double[cols];
		
		for (int i = 0; i < cols; i ++) {
			greatestColNumber[i] = Double.MIN_VALUE;
		}
		
		forEach((row, col, value) -> {
			if (greatestColNumber[col] < value) {
				greatestColNumber[col] = value;
				result.a[col] = row;
			}
		});
	...
	public Matrix addIncrement(int row, int col, double increment) {
		...
		result.set(row, col, result.get(row, col)+increment);
		return result;
	}
